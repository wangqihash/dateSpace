class City {
    ...
    bindEvent() {
        ...
        $(this.component).on('click','[city]',function () {
            // wq 这样可以理解为什么 每个城市的都有 city=""这样的属性，是为在这匹配
            // wq 加深理解回调函数 [调用，逻辑实现函数] => 明确那块是调用，那块是实现，
            // wq index.js中调用的只是 show方法， 而show需要回调，回调在index里只是传递没有调用
            // wq ==>配合理解 avalon中组件之间的传值交互
    ...
    render() {
        //获取有数据生成的字母表，渲染dom
        let alpha = this.getAlphabet(); // wq: return 将右侧字母排序后的数组
        let str='',str2='',str3='';     // str => 承接右侧字母, str2 => 热门城市, str3 => 全部城市列表
    ...
    getAlphabet(){
        ...
        data.forEach((v,i)=>{
            ...
            // 这段逻辑 将 开头字母相同的城市都放到了 alphaobj对象中
            // 将每个字母大写且都存在 alphabet数组中
