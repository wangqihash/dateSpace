...
class Paddle {
  ...
  collide (ball) {
    let b = ball
    let p = this
    //碰撞(如果碰撞了肯定在这个范围内) 这里用的是绝对值,所以不管在左右都符合条件
    // 如果加上 <=  那刚好就是擦边
...
class Ball {
  ...
  //这里的game是由于 main.js中 new Score(self)传递并执行了方法，所以这里也能取到值
  move (game) {
    if (this.fired) {
      // 碰撞边界检测
...
class Score {
  ...
  computeScore () {
    ...
    // blockListLen: 集合的总数是不会变的， blockList：砖块对象会随着每次的碰撞而减少，
    //  allScore：总分数也就增加
...
class Scene {
  ...
  creatBlockList () {
    let lv = this.lv,                         // 游戏难度级别
        ...
        //wq 这里固定给50， 而canvas的宽度为1000
        //这么算下来最多为20个而第一关的第一层最大为16个，补齐4个(左右各2个)刚好就是这个数
    ...
    switch (lv) {
      case 1 : // 正三角形
        ...
        for(let i = 0;i < yNum;i++){
          ...
          // xNum_max == 20  x_start 的值根据  xNum 而改变
          // 根据 砖块最左边的为0px 而不是更具canvas的最左边来计算的
