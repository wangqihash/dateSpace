(function(window, $) {
  ...
  var proto = {
    ...
    setCenter: function(){
      var self = this;
      this.nb.centerX = self.bn.left + self.nb.mWidth*self.nb.nScale/2;
      this.nb.center = self.nb.top + self.nb.mHeight*self.nb.nScale/2;
    },
    //对外提供，改变图片大小
    setScale: function(stc, callback){
      var self = this;
      if(str = 'plus'){
        self.nb.nScale += 0.1;
      }else if(str == 'reduce'){
